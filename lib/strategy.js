...
Strategy.prototype.authenticate = function(req, options): any {
    ...
    if (req.session && req.session.lang) {
        ...
        if (this._oauth2._authorizeUrl.includes('lang=')) {
            ...
            const indexOfLang = this._oauth2._authorizeUrl.indexOf('lang=');
            const baseUrl = this._oauth2._authorizeUrl;
    ...
    if (req.session && req.session.amplitudeDeviceId) {
        ...
        if (this._oauth2._authorizeUrl.includes('amplitudeDeviceId=')) {
            // getting the starting index of the language in order to override
            const indexOfamplitudeDeviceId = this._oauth2._authorizeUrl.indexOf('amplitudeDeviceId=');
            const baseUrl = this._oauth2._authorizeUrl;
            this._oauth2._authorizeUrl = baseUrl.substr(0, indexOfamplitudeDeviceId + 18) + req.session.amplitudeDeviceId + baseUrl.substr(indexOfamplitudeDeviceId + 18 + req.session.amplitudeDeviceId.length);
        } else {
            if (req.session.lang) {
